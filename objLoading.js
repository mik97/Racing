function parseOBJ(text) {
  ...
  function setGeometry() {
    if (!geometry) {
      ...
      const color = [];
      const exact = [];
      webglVertexData = [position, texcoord, normal, color, exact];
      geometry = {
        object,
        groups,
        material,
  ...
  function addVertex(vert) {
    ...
    geometry.data.exact = exactPosition;
  ...
  const keywords = {
    v(parts) {
      ...
      exactPosition.push(parts.map(parseFloat));
    },
    vn(parts) {
...
function createTexture(gl, url) {
  const texture = create1PixelTexture(gl, [0, 0, 255, 255]);
